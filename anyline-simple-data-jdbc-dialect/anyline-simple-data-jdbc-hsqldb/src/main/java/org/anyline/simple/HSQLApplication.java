...
public class HSQLApplication {
	...
	public static void main(String[] args) {
		SpringApplication application = new SpringApplication(HSQLApplication.class);
		ConfigurableApplicationContext context = application.run(args);
		org.slf4j.impl.StaticLoggerBinder s;
		AnylineService service = (AnylineService) context.getBean("anyline.service");
		String sql = "${DROP TABLE IF EXISTS hr_user ; CREATE TABLE hr_user(ID INT, NAME VARCHAR(20))}";
		service.execute(sql);

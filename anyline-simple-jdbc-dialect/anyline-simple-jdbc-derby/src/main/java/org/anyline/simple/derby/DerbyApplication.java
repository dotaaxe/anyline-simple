...
public class DerbyApplication {
    ...
    public static void ddl() throws Exception{
        ...
        //检测表结构
        Table table = service.metadata().table(catalog, schema,DerbyApplication.table);
        //如果存在则删除
        if(null != table){
            service.ddl().drop(table);
        }
        //也可以直接删除(需要数据库支持 IF EXISTS)
        //service.ddl().drop(new Table(catalog, schema,DerbyApplication.table));

        //再查询一次
        table = service.metadata().table(catalog, schema,DerbyApplication.table);
        //Assertions.assertNull(table);

        //定义表结构
        table = new Table(catalog, schema,DerbyApplication.table);

        //添加列
        //自增长列 如果要适配多种数据库 setAutoIncrement 有必须的话可以设置起始值与增量 setAutoIncrement(int seed, int step)
        table.addColumn("ID", "BIGINT", false, null).setComment("主键").setAutoIncrement(true).setPrimaryKey(true);
